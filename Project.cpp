...
class Ability{
    ...
    public:
        ...
        virtual int UseAbility(Pet*& pet)
        {
            return 0;
        }
...
class Pet{
    ...
    public:
        ...
        virtual void Train(int tp)
        {
            trainingPoints = tp;
        }

        virtual void Attack1(Pet*& other)
        {

        }

        virtual void ResetAttack(Pet*& other)
        {
            
        }

        virtual int Attack2(Pet*& other)
        {
            return 0;
        }
...
class SpeedBurst: public Ability{
    ...
    public:
        ...
        int UseAbility(Pet*& pet)
        {
            pet->setSpd(pet->getSpd() + speedIncrement);
            return time;
        }
        void ResetAbility(Pet*& pet)
        {
...
class Griffin: public Pet{
    private:
        ...
        DiveBomb ability2;
    public:
        Griffin(string type = "NULL", string name = "Griffin", int Health = 100, int Attack = 15, int Speed = 20, int Defense = 25, bool isAlive = true, int trainingPoints = 0, int level = 0):Pet(type, name, Health, Attack, Speed, Defense, isAlive, trainingPoints, level), ability1(15 + 0.1*Attack), ability2(10, 10 + 0.1*Attack)
        {
        ...
        void Attack1(Pet*& other)
        {
            ability1.UseAbility(other);
        }

        int Attack2(Pet*& other)
        {
            return ability2.UseAbility(other);
        }

        void ResetAttack(Pet*& other)
        {
...
class Pheonix: public Pet{
    private:
        FlameHealing ability1;
        Rebirth ability2;
    public:
        Pheonix(string type = "NULL", string name = "Pheonix", int Health = 150, int Attack = 15, int Speed = 10, int Defense = 50, bool isAlive = true, int trainingPoints = 0, int level = 0):Pet(type, name, Health, Attack, Speed, Defense, isAlive, trainingPoints, level), ability1(20, 5 + 0.1*Attack), ability2(false)
        {
...
class Dragon: public Pet{
    private:
        ...
        DragonRoar ability2;
    public:
        Dragon(string type = "NULL", string name = "Dragon", int Health = 200, int Attack = 30, int Speed = 15, int Defense = 25, bool isAlive = true, int trainingPoints = 0, int level = 0):Pet(type, name, Health, Attack, Speed, Defense, isAlive, trainingPoints, level), ability1(30 + 0.1*Attack), ability2(35 + 0.1*Attack)
        {
...
class Unicorn: public Pet{
    private:
        SpeedBurst ability1;
        MagicShield ability2;
    public:
    Unicorn(string type = "NULL", string name = "Unicorn", int Health = 50, int Attack = 20, int Speed = 30, int Defense = 25, bool isAlive = true, int trainingPoints = 0, int level = 0):Pet(type, name, Health, Attack, Speed, Defense, isAlive, trainingPoints, level), ability1(15, 15 + 0.1*Attack), ability2(20 + 0.1*Defense)
        {
...
class ObstacleCourse: public TrainingCamp{
    ...
    public:
        ...
        {
            pet->Train(pet->getTrainingPoints() + trainingPoints);
    ...
    public:
        ...
        void Battle(Pet* pet, int*& playerItems, RenderWindow& window)
        {
            ...
            while(window.isOpen())
            {
                ...
                if(!attackReady)
                {
                    attackCooldown = attackCooldown - cooldownClock.getElapsedTime().asSeconds();
                }

                if(attackCooldown < 0 && !attackReady)
                {
                    attackReady = true;
                    isHealing = false;
                    if(pet->getName() == "Unicorn" && isSpeeding)
                    {
                        pet->ResetAttack(pet);
                        petStats[4].setString("Speed: " + to_string(pet->getSpd()));
                        isSpawning = false;
                ...
                {
                    if(bot.calculateDistance(petSprite.getPosition(), botpetSprite.getPosition()) < 250)
                    {
                        if(pet->getName() == "Griffin" && attackReady)
                        {
                            pet->Attack1(botPet);
                            botPetStats[1].setString("Health: " + to_string(botPet->getHp()));
                            attackCooldown = 25.0f;
                            attackReady = false;
                            cooldownClock.restart();

                            AttackTexture.loadFromFile("ClawStrike.png");
                            AttackSprite.setTexture(AttackTexture);
                            AttackSprite.setPosition(botpetSprite.getPosition());
                        }
                        if(pet->getName() == "Pheonix" && attackReady)
                        {
                            pet->Attack1(pet);
                            petStats[1].setString("Health: " + to_string(pet->getHp()));
                            attackCooldown = 25.0f;
                            ...
                            AttackTexture.loadFromFile("FlameHealing.png");
                            AttackSprite.setTexture(AttackTexture);
                            AttackSprite.setPosition(petSprite.getPosition());
                        }
                        if(pet->getName() == "Dragon" && attackReady)
                        {
                            targetPos = botpetSprite.getPosition();
                            attackCooldown = 25.0f;
                            attackReady = false;
                            cooldownClock.restart();

                            AttackTexture.loadFromFile("Fireball.png");
                            AttackSprite.setTexture(AttackTexture);
                            AttackSprite.setPosition(petSprite.getPosition());
                        }
                        if(pet->getName() == "Unicorn" && attackReady)
                        {   
                            pet->Attack1(pet);
                            petStats[4].setString("Speed: " + to_string(pet->getSpd()));
                            attackCooldown = 25.0f;
                            ...
                            AttackTexture.loadFromFile("Speed Burst.png");
                            AttackSprite.setTexture(AttackTexture);
                            AttackSprite.setPosition(petSprite.getPosition());
                        }
                    }
                ...
                {
                    if(bot.calculateDistance(petSprite.getPosition(), botpetSprite.getPosition()) <= 200)
                    {
                        if(pet->getName() == "Griffin" && attackReady)
                        {
                            pet->Attack2(botPet);
                            botPetStats[1].setString("Health: " + to_string(botPet->getHp()));
                            attackCooldown = 50.0f;
                            attackReady = false;
                            canMove = false;
                            cooldownClock.restart();
                            
                            AttackTexture.loadFromFile("BombDive.png");
                            AttackSprite.setTexture(AttackTexture);
                            AttackSprite.setPosition(botpetSprite.getPosition());
                        }
                        if(pet->getName() == "Dragon" && attackReady)
                        {
                            botPetStats[1].setString("Health: " + to_string(botPet->getHp()));
                            attackCooldown = pet->Attack2(botPet);
                            attackReady = false;
                            canMove = false;
                            cooldownClock.restart();
                            
                            AttackTexture.loadFromFile("DragonRoar.png");
                            AttackSprite.setTexture(AttackTexture);
                            AttackSprite.setPosition(botpetSprite.getPosition());
                        }
                    }
                ...
                for(int i = 0; i < numOfObstacles; i++)
                {
                    ...
                    if(currentObs[i] == 1)
                    {
                        ...
                        if(checkCollision(botpetSprite, obsS[i]))
                        {
                            botPet->setHp(botPet->getHp() - 0.001);
                            botPetStats[1].setString("Health: " + to_string(botPet->getHp()));
                        }
                    }
                }
                ...
                if(attackReady == false)
                {
                    if(pet->getName() == "Dragon")
                    {
                        AttackSprite.move(bot.calculateMoveDir(petSprite.getPosition(), targetPos).x*0.01f, bot.calculateMoveDir(petSprite.getPosition(), targetPos).y*0.01f);
                        if(checkCollision(AttackSprite, botpetSprite))
                        {
                            pet->Attack1(botPet);
                            botPetStats[1].setString("Health: " + to_string(botPet->getHp()));
                        }
                    ...
                    {
                        AttackSprite.setPosition(petSprite.getPosition());
                        if(checkCollision(petSprite, botpetSprite))
                        {
                            botPet->setHp(botPet->getHp() - pet->getAtk()*0.01);
                            botPetStats[1].setString("Health: " + to_string(botPet->getHp()));
                        }
                    }
                            ...
                            if(upgradeButtonBounds[0 + 4*i].contains(mousePosition.x, mousePosition.y))
                             {
                                ...
                                player.getPets()[i]->Train(player.getPets()[i]->getTrainingPoints() - 1);
                                break;
                            }
